...
def eval_sub_data(model, sub_data, answer_map):
    ...
    for i in range(lt):
        if PRED[i]:
            ...
            #random choice for postproc
            if res['opt'] == 'Z':
                options = list(build_choices(sub_data.iloc[i]))
                res['opt'] = rd.choice(options)
                res['log'] = 'Randomly choice as the postproc for Z'
...
def postproc_eval(eval_file, dataset='default', **judge_kwargs):
    logger = get_logger('Evaluation')
    ...
    result_file = eval_file.replace(f'.{suffix}', f'_{name_str}_postproc_result.pkl')
    result = {}
    ...
    # load split
    dump(data_main, eval_file.replace(f'.{suffix}', f'_{name_str}_postproc_result.{suffix}'))
    data_main = load(eval_file.replace(f'.{suffix}', f'_{name_str}_postproc_result.{suffix}'))

    acc = report_acc(data_main)
    score_file = eval_file.replace(f'.{suffix}', '_postproc_acc.csv')
    dump(acc, score_file)
